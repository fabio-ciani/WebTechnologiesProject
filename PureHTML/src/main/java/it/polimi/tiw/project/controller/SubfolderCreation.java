...
public class SubfolderCreation extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if (name.length() > 64) {
				response.sendRedirect(getServletContext().getContextPath() + "/manager?subfolderLengthError=true");
				return;
			}
		} catch (Exception e) {
		...
		try {
			...
			if (f.checkSubfolderAvailability(name, parent_folder, u.getUserID())) {
				response.sendRedirect(getServletContext().getContextPath() + "/manager?subfolderNameError=true");
				return;
			}
		...
		*/
			...
			response.sendError(HttpServletResponse.SC_INTERNAL_SERVER_ERROR,
					"An error has occured: the subfolder couldn't be created");
			return;
		}
